...
function getArtistByIndex(artistArray, number) {
  let artist = `the artist at index ${number} is ${artistArray[number].name}`;
  return artist;
}
...
function listOfNames(artistArray) {
  let newArtistArray = [];
  // Add each artist name to the new Array
...
function removeArtist(artistArray, number) {
  let newArtistArray = [];
  // Copies all but the desired index artist to a new list
  for (let artist in artistArray) {
    if (artist != number) {
      newArtistArray = newArtistArray.concat(artistArray[artist]);
    }
  }
  return newArtistArray;
}
...
function addArtist(artistArray, name, years, genre, nationality, bio) {
  let newArtistArray = artistArray;
  ...
  // Add the new Artist to Artist Array
  newArtistArray = newArtistArray.concat(newArtist);
  return newArtistArray;
}
console.log(addArtist(artists, "Isaac", "2001 - Present Day", "Web Design", "American", "A super cool, future Software Engineer"));
...
function lotsOfArt(artistArray) {
  let newArtistArray = [];
  // If artist has over 100 paintings, Add it to the new Array
  for (let artist in artistArray) {
    if (artistArray[artist].paintings > 100) {
      newArtistArray = newArtistArray.concat(artistArray[artist].name);
    }
  }
  return newArtistArray;
...
function artistInfo(artistArray, name){
  let artistBio = "";
  // Search for artist name & return their Bio
  for (let artist in artistArray) {
    if (artistArray[artist].name === name) {
      artistBio = artistArray[artist].bio;
    }
  }
  return artistBio
}
